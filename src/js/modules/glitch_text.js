import React from 'react'

export default class GlitchText extends React.Component{
	constructor(props){
		super(props);	
		this.draw=this.draw.bind(this);
		this.randomRange=this.randomRange.bind(this);

		this.onMouseEnter=this.onMouseEnter.bind(this);
		this.onMouseLeave=this.onMouseLeave.bind(this);

		this._options={
	        color:{
	            red:1,
	            green:0.8,
	            blue:0.58
	        },
	        lineOffset:{
	            value:2,
	            lineHeight: 5
	        },
	        frame_rate:100
    	};
    	this.state={
    		glitch:false
    	};
    	this.density=1.0;

	}
	componentDidMount(){	
		this.intervalId=setInterval(this.draw,this._options.frame_rate);
		if(!this.props.hover) this.setState({'glitch':true});
	}
	draw(){		
		
		
		this.refs._canvas.width=this.props.font_size*this.props.text.length;
		this.refs._canvas.height=this.props.font_size*1.5;

		const ctx=this.refs._canvas.getContext('2d');

		let w_=this.refs._canvas.width;
		let h_=this.refs._canvas.height;



		ctx.fillStyle="transparent";
		ctx.clearRect(0,0,w_,h_);
		ctx.fillStyle="white";
		ctx.textBaseline="bottom";
		ctx.font=this.props.font_size+'pt Assistant';		
 		ctx.fillText(this.props.text, 0, h_);
		
		
		//if(this.randomRange(1,10)>1) return;
		if(!this.state.glitch) return;
		// if(this.density<0) return;
		//if(!this.props.hover && this.randomRange(1,5)>1) return;
		
		var imageData=ctx.getImageData(0,0,w_,h_),
			pixels=imageData.data,
			length=pixels.length,
			offset,i,x,y;

		if(!this.props.hover) offset=this._options.lineOffset.value;
		else
    		offset=this.randomRange(1,this._options.lineOffset.value);
    	

    	var turb=0;
    	for(y=0;y<h_;++y){
    		if(this.props.hover){
    			if(y%this._options.lineOffset.lineHeight==0) 
    			if(this.randomRange(1,5)<2) turb=this.randomRange(-1,1);
    			else turb=0;
    		}
    		for(x=0;x<w_;++x){  
    			
    			let off=offset+turb;
    			//console.log(off);
    			
    			if(this.randomRange(0,50)<1) off+=this.randomRange(-2,2);
    			
				let val=pixels[(x+y*w_)*4];
				var cr=(x+off>=w_)?0:pixels[(x+off+y*w_)*4];
				var cg=(x-off<0)?0:pixels[(x-off+y*w_)*4];

				pixels[(x+y*w_)*4]=val+cr;
				pixels[(x+y*w_)*4+1]=val+cg;
				pixels[(x+y*w_)*4+2]=val+cr+cg;
				pixels[(x+y*w_)*4+3]=(val+cr+cg);

			
    		}
    	}

		this.density-=1.0/(this.props.last/100.0);
		
		ctx.putImageData(imageData,0,0);
	}
	onMouseEnter(){
		if(this.props.hover) this.setState({'glitch':true});		
	}
	onMouseLeave(){		
		if(this.props.hover) this.setState({'glitch':false});		
	}
	randomRange(min, max){
    	return Math.floor(Math.random()*(max-min+1))+min;
	}
	render(){
		return (
			<canvas ref="_canvas" 
					className={this.props.className}
					onMouseEnter={this.onMouseEnter}
					onMouseLeave={this.onMouseLeave}/>			
		);
	}
}